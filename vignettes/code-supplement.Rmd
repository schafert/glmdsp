...
Additionally, we can compare to the independent horseshoe fit meaning there is no dependence among the latent local variance terms.


```{r nb_hs}
fit_nb_hs1 <- btf_nb(y = y, 
                     D = 1, 
                     evol_error = "HS",
                     nburn = 5000,
                     evol0_sample = FALSE,
...
fit_gau_dhs1 <- btf(y = y,
                    D = 1,
                    nburn = 5000,
                    verbose = FALSE
)

fit_gau_dhs2 <- btf(y = y,
                    D = 2,
                    nburn = 5000,
                    verbose = FALSE
)
...
```{r log_dhs}
fit_log_dhs1 <- btf(y = log(y + 1),
                    D = 1,
                    nburn = 5000,
                    verbose = FALSE
)

fit_log_dhs2 <- btf(y = log(y + 1),
                    D = 2,
                    nburn = 5000,
                    verbose = FALSE
)
...
fit_glm1 <- trendfilter(y, 
                   k = 1-1,
                   family = "poisson")

fit_glm2 <- trendfilter(y, 
                   k = 2-1,
                   family = "poisson")
```
